...
def load_tower_light_config_from_db(logger=None) -> Optional[TowerLightConfig]:
    ...
    return TowerLightConfig(
        ...
        silence_enabled=bool(settings.get("silence_enabled", True)),
        silence_color=_color("silence_color", "magenta"),
        silence_buzzer=bool(settings.get("silence_buzzer", False)),
        gate_pending_enabled=bool(settings.get("gate_pending_enabled", True)),
        gate_pending_color=_color("gate_pending_color", "blue"),
