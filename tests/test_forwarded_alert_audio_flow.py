...
class TestBuildFilesHasNoEmbeddedLeadInSilence:
    """build_files() must NOT embed a silent lead-in into the composite
    audio. An earlier version of this fix embedded 1s of silence directly
    into the 'same' WAV segment, mirroring the (also since-removed)
    audio-embedded trailing silence -- but that broke the moment a stored
    alert was resent: resend replays the exact stored bytes rather than
    regenerating audio, so the padding was frozen at generation time and
    could never apply to old messages, and every consumer of the stored
    audio (Icecast listeners, FCC-compliance exports, archived recordings)
    got artificial dead air baked into the actual alert content. The relay
    lead-in/lead-out is now a program-level GPIO timing concern applied by
    every caller that drives the airchain (BROADCAST_LEAD_IN_SECONDS /
    BROADCAST_LEAD_OUT_SECONDS in app_utils/eas.py) -- this guards the
    audio-generation side of that: no embedded padding, ever.
    """
    ...
    def test_same_segment_opens_with_header_energy_immediately(self):
        gen = _build_generator()
        alert = _build_minimal_alert()
        payload = _build_payload()
        header = 'ZCZC-WXR-RWT-039137+0100-0011200-EASNODES-'
        ...
        first_window = samples[:int(rate * 0.05)]
        peak = max(abs(s) for s in first_window)
        assert peak > 1000, (
            f"'same' segment must open with the header FSK burst "
            f"immediately (peak amplitude {peak} too low) -- no silent "
            "lead-in belongs in the audio itself."
        )

    def test_main_audio_has_no_trailing_silence_only_tail(self):
        """When no post-alert chime is configured, the composite must end
        at the true end of content (EOM) rather than an extra second of
        bare silence tacked on after it."""
        gen = _build_generator()
        alert = _build_minimal_alert()
        payload = _build_payload()
        header = 'ZCZC-WXR-RWT-039137+0100-0011200-EASNODES-'
        ...
        # Everything before the EOM (same/attention/buffer segments) plus the
        # EOM itself should account for the composite, with no more than a
        # fraction of a second of floating-point slack -- not a full extra
        # second of unconditional trailing silence.
        accounted = sum(
            _parse_wav_duration(seg['wav_bytes'])
            for key, seg in segment_payload.items()
            if key not in ('composite',) and seg.get('wav_bytes')
        )
        assert main_duration <= accounted + 0.05, (
            f"Composite duration ({main_duration:.2f}s) exceeds the sum of "
            f"its known segments ({accounted:.2f}s) by more than rounding "
            "slack -- an unconditional trailing silence tail may have crept "
            "back into the audio."
        )
