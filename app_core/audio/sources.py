...
def create_audio_source(config: AudioSourceConfig) -> AudioSourceAdapter:
    ...
    if config.source_type.value == "sdr":
        # The direct-hardware SDRSourceAdapter this used to construct was
        # removed as dead code: every SDR-backed AudioSourceConfigDB row is
        # created exclusively by webapp/admin/audio_ingest/radio_sources.py,
        # which always sets managed_by='radio' -- eas_monitoring_service.py's
        # loader short-circuits on that flag and constructs a
        # RedisSDRSourceAdapter directly, never reaching this factory. A
        # source_type='sdr' row without managed_by='radio' is a malformed
        # config, not a supported case.
        raise RuntimeError(
            "SDR sources are managed via RadioReceiver + RedisSDRSourceAdapter "
            "(see webapp/admin/audio_ingest/radio_sources.py); a plain "
            "AudioSourceConfigDB row of type 'sdr' is not a supported "
            "configuration."
        )
