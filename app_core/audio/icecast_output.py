...
class IcecastStreamer:
    ...
    def _extract_metadata_fields(
        ...
        Thin wrapper around the module-level ``extract_now_playing_fields``
        (see now_playing_metadata.py) -- kept as an instance method for
        backward compatibility with existing callers/tests; the webapp
        process's public now-playing API imports the shared function
        directly rather than pulling in this whole module (numpy/requests/
        subprocess, all needed for FFmpeg process management but not for
        parsing a metadata dict).
        """
        return extract_now_playing_fields(metadata)
