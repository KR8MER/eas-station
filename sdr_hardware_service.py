...
from app_core.radio.spectrum import (
    compute_spectrum,
    FFT_SIZE,
    FFT_MIN_MAGNITUDE,
    MIN_SPECTRUM_DB_SPAN,
)
...
# Spectrum computation constants (FFT_SIZE, FFT_MIN_MAGNITUDE,
# MIN_SPECTRUM_DB_SPAN) and compute_spectrum() itself now live in
# app_core/radio/spectrum.py, shared with the MPX spectrum computed in
# app_core/radio/demod/fm.py -- imported above, still usable here as
# module-level names (and as sdr_hardware_service.compute_spectrum,
# preserving every existing caller/test).
...
def verify_soapysdr_installation():
    ...
    try:
        ...
        logger.error("     apt-cache policy python3-soapysdr")
        logger.error("")
        logger.error("  2. If not available, build SoapySDR Python bindings from source:")
        ...
        logger.error("     cmake .. -DPYTHON3_EXECUTABLE=/usr/bin/python3")
