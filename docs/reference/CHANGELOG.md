...
- **Admin FIPS picker surfaces marine areas** loaded via `mz_*.dbf`. `app_utils/fips_codes.py` gained `MARINE_PREFIX_TO_SAME_STATE` (UGC prefix → SAME `SS` digits — only entries verified against on-air SAME headers are enabled), `get_marine_state_tree()` (queries `nws_zones` and emits state-shaped entries with 6-digit `PSSCCC` `code` values per marine area), and `get_extended_state_county_tree()` which composes the static US tree with the runtime marine tree. `webapp/admin/dashboard.py` now feeds the admin template from the extended version so the State/County dropdown can show e.g. *Gulf of Mexico Marine → Coastal waters from Pensacola FL to Pascagoula MS out 20 NM (077650)* and operators can add marine SAME codes through the normal picker UI instead of having no way to monitor marine alerts at all. The Gulf-of-Mexico prefix mapping (`GM` → `77`) was verified byte-for-byte against an on-air SMW header carrying `077650 / 077633 / 077632 / 077631` for "Pensacola–Pascagoula coastal / Perdido Bay / Mississippi Sound / South Mobile Bay" — those CCC values correspond exactly to GMZ650, GMZ633, GMZ632, GMZ631 in the NWS marine zone DBF. Other marine prefixes (PH, PK, AM, AN, LM, LS, LE, LO, LH, LC, PM, PZ, PS, SL) are left out of the default mapping until each is similarly verified against an authoritative SAME header or NWS reference file. The base `get_us_state_county_tree()` remains DB-free so import-time callers (`app_core/location.py:70`, `app_core/alert_filtering.py:46`) are untouched.

### Changed
- **`sync_zone_catalog` is now schema-scoped** (`delete_scope="public" | "marine" | None`). The previous full-replace behaviour meant uploading a marine DBF after a public DBF — or the reverse — wiped every previously-loaded zone, since orphans (anything not in the incoming file) were deleted unconditionally. The new admin upload route inspects the DBF with the new `detect_zone_schema()` helper and passes the matching scope so public and marine catalogs are additive. `current_app.config['NWS_ZONE_DBF_PATH']` is only updated for public-zone uploads so a marine upload doesn't redirect the on-restart auto-load to a file that lacks terrestrial zones.
