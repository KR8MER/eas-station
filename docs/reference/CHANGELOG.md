...
### Fixed
- **GPS Live Status panel now shows qualitative signal labels and time-series charts** — satellite table gains a "Quality" column (Strong ≥31 dBHz / Fair 20–30 / Weak <20) with a colored dot, matching industry thresholds. The SNR legend is updated to show these three tiers. Below the live panel, two new canvas charts auto-populate as data arrives every 2 seconds: "Satellite Count" (Visible and Used lines with a 4-satellite minimum threshold) and "Signal Quality" (Max/Avg/Min SNR lines with a 25 dBHz good-signal threshold). Both charts are theme-aware and show a rolling ~5-minute history with HH:MM:SS time labels. — the satellite sky view canvas uses a sky-blue radial gradient (pale zenith → deep navy horizon) with a horizon haze band, white semi-transparent elevation rings and crosshairs, and white labels to match the appearance of standard GPS sky-view tools. Satellite dots use SNR-based colors with a glow ring on fix-contributing satellites. Cardinal labels (N/S/E/W) outside the sky circle use the theme `--text-color` so they remain readable across all themes. — all hardcoded GitHub dark-mode hex colors in the GPS panel CSS and inline JavaScript have been replaced with CSS custom properties (`var(--bg-color)`, `var(--surface-color)`, `var(--text-color)`, `var(--border-color)`, `var(--text-muted)`, `var(--primary-soft)`). The sky-plot canvas also reads theme colors at draw-time so rings, crosshairs, labels, and cardinal marks match the current theme instead of always rendering in dark mode.
