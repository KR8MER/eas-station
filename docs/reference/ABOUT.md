...
- **Upgrade & Backup Tooling:** Use `python tools/create_backup.py` for pre-flight snapshots and `sudo bash update.sh` to roll forward in place. The Admin console exposes one-click buttons for both tasks under System Operations; the upgrade button runs this exact script as its own systemd unit and streams its progress back to the page.
