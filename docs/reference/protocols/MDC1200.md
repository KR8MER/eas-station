...
[radio PTT keys up by GPIO/VOX]
  Pre  MDC1200 PTT-ID-Pre   (op 0x01, arg 0x80, unit_id = your station)
  500 ms silence                                  ← lets the MDC PLL settle
  SAME header burst × 3                           ← decoded by ENDECs/SAME radios
  1050 Hz attention tone (8 s)
  Voice narration (TTS or operator audio)
  SAME EOM × 3
  500 ms silence
  Post MDC1200 PTT-ID-Post  (op 0x00, arg 0x80, unit_id = your station)
[radio PTT releases]
```
...
```
┌──────────────────┬──────────────────┬─────────────────────────────┐
│  Preamble        │  Frame Sync      │  Payload                    │
│  3 bytes         │  5 bytes         │  14 bytes                   │
│  0x00 0x00 0x00  │  07 09 2A 44 6F  │  (info + FEC, interleaved)  │
└──────────────────┴──────────────────┴─────────────────────────────┘
```
...
```
   ...
   │  ┌────┬────┬────┬────┬─────┬─────┬────────┐ │
   │  │ op │arg │idH │idL │crcL │crcH │ status │ │
   │  └────┴────┴────┴────┴─────┴─────┴────────┘ │
   └─────────────────────────────────────────────┘
                        │
                        ▼  K=7 / R=1/2 convolutional encoder
   ┌─────────────────────────────────────────────┐
   │  14-byte FEC block (info + 7 parity bytes)  │
   └─────────────────────────────────────────────┘
                        │
                        ▼  16×7 bit interleaver
   ┌─────────────────────────────────────────────┐
   │  14-byte interleaved payload (= "Payload")  │
   └─────────────────────────────────────────────┘
```
...
#### 3.3.3 K=7 rate-1/2 convolutional FEC
...
#### 3.3.5 Differential ("XOR") modulation
