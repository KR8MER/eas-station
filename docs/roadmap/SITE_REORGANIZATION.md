...
| 4 | `templates/admin/data_management.html` | Its "Zone Catalog" tab called the *exact same* `/admin/zones/*` endpoints (`webapp/admin/zones.py`) as the pre-existing, separately nav-registered `/admin/zones` page -- confirmed by grepping for every route string these two templates' JS calls, finding exactly one backend implementation of each. `dashboard.py`'s `data_management_page()` docstring had claimed, from an earlier extraction, that this was "confirmed not a duplicate" -- true then or not, it no longer held up under inspection. | Zone Catalog tab removed entirely (linked out to the pre-existing `/admin/zones` instead — no new page needed, same pattern as Phase 2's Boundary Coverage card); `data_management.html` keeps its Boundaries + Manage tabs, which really are one cohesive workflow (upload a boundary type, then browse/filter/delete it). Docstring and `NavItem` description corrected. | **Done.** |

### Investigated and resolved (not duplicates)

- `data_management.html`'s "Boundaries" tab lets an operator upload a boundary type of `county` through its generic type dropdown, which looked like it might overlap with `/admin/county_boundaries`' dedicated county-boundary loader. Checked the actual models both write to: the generic uploader writes `app_core.models.Boundary` (table `boundaries`, arbitrary `type` string, no FIPS structure — one row per uploaded polygon of whatever type the operator labels it), while `/admin/county_boundaries` writes `app_core.models.USCountyBoundary` (table `us_county_boundaries`, FIPS/GEOID-keyed, loaded from Census TIGER/Line shapefiles specifically for SAME-code multi-county alert geometry union). Different tables, different schemas, different purpose — "county" is just one of many generic overlay-type labels in the first uploader's dropdown, unrelated to the canonical reference dataset the second page manages. **Not a duplicate; nothing to change.**
...
- **Phase 2** — done.
- **Phase 3** — done.
- **Phase 4** — done. All phases in this document's original scope are complete; new candidates would need their own phase entries.
