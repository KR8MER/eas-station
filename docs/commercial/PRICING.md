# EAS Station Commercial Licensing - Pricing Structure

**Version:** 1.0 (Alpha Pre-Release)
**Effective Date:** November 2025
**Copyright:** 2025 Timothy Kramer (KR8MER)

---

## Important Notice

**ALPHA SOFTWARE DISCLAIMER**

EAS Station is currently in **alpha stage development** and is:
- ❌ **NOT FCC-certified** for production use
- ❌ **NOT approved for commercial emergency alerting**
- ✅ **Suitable for research, development, and testing**
- ✅ **Laboratory and controlled environment use only**

Commercial licenses are available NOW to lock in early adopter pricing, but production deployment must wait for FCC certification completion (estimated 2026).

---

## Commercial License Overview

EAS Station is dual-licensed:
- **Open Source (AGPL v3)**: Free for open-source projects with source disclosure requirements
- **Commercial License**: For proprietary/closed-source use without copyleft obligations

---

## Pricing Tiers

### 1. Single Station License
**$995/year** or **$2,995 perpetual**

**Includes:**
- Single production installation
- Commercial use without AGPL obligations
- Standard technical support (email, 48-hour response)
- Security patches and bug fixes
- Version upgrades during active subscription
- Integration with proprietary systems

**Ideal for:**
- Individual radio/TV stations
- Small amateur radio clubs
- Single location deployments

**Value:** Comparable to DASDEC hardware ($3K-$7K) but with software flexibility and no vendor lock-in.

---

### 2. Multi-Station License (Up to 10 Installations)
**$4,995/year** or **$12,995 perpetual**

**Includes:**
- Up to 10 production installations
- All Single Station benefits
- Priority technical support (24-hour response)
- Quarterly feature consultation calls
- Custom configuration assistance
- Multi-site coordination setup

**Ideal for:**
- Regional broadcast groups
- Emergency management agencies
- Multi-location organizations
- Amateur radio networks

**Value:** $1,300 per installation (perpetual) vs. $5,000+ per DASDEC unit = **70% cost savings**.

---

### 3. Enterprise License (Unlimited Installations)
**$19,995/year** or **$49,995 perpetual**

**Includes:**
- Unlimited production installations
- All Multi-Station benefits
- Premium support (12-hour response, phone/video)
- Monthly strategic consultation calls
- Custom feature development (up to 40 hours/year)
- White-label/rebranding options (with attribution)
- Source code access for internal modifications
- FCC certification documentation assistance

**Ideal for:**
- National broadcast networks
- State/federal emergency management
- Large amateur radio organizations
- Commercial alert distribution services

**Value:** For 10+ stations, saves $40K-$60K vs. buying individual DASDEC units.

---

### 4. OEM/Integration License
**Starting at $25,000** (custom pricing)

**Includes:**
- Embedding EAS Station in your hardware/software products
- Redistribution rights to your customers
- Custom branding (with attribution retained in about/credits)
- Dedicated integration engineering support
- Co-marketing opportunities
- Revenue sharing options available
- Joint FCC certification assistance

**Ideal for:**
- Hardware manufacturers (broadcast equipment)
- Software platform providers
- System integrators
- Emergency communications equipment vendors

---

## Add-On Services

### Professional Services
- **Custom Development:** $150/hour (20-hour minimum project)
- **FCC Certification Assistance:** $10,000 - $25,000 (depending on scope)
- **On-Site Installation & Training:** $1,500/day + expenses
- **24/7 Critical Support:** +$5,000/year per license

### Extended Support Tiers
- **Gold Support:** +$1,000/year (8-hour response, phone support)
- **Platinum Support:** +$2,500/year (4-hour response, dedicated engineer)
- **Mission Critical:** +$7,500/year (1-hour response, 24/7/365 availability)

---

## Payment Terms

### Annual Subscriptions
- Billed annually in advance
- Auto-renewal (can be cancelled 30 days before renewal)
- Includes all updates and support for subscription period
- Volume discounts available for multi-year commitments:
  - 2 years: 10% discount
  - 3 years: 15% discount
  - 5 years: 20% discount

**Example:** Single Station 3-year prepay = $995 × 3 × 0.85 = **$2,537 total** ($846/year)

### Perpetual Licenses
- One-time payment
- Lifetime license to current major version
- Includes 1 year of updates and support
- Optional maintenance renewal: $199/year (Single), $599/year (Multi-Station), $2,499/year (Enterprise)
- Major version upgrades: 50% discount on new perpetual license

### Payment Methods
- Wire transfer (ACH/SWIFT)
- Corporate check
- Credit card (+3% processing fee)
- Net 30 terms available for enterprise customers with credit approval

---

## Educational & Non-Profit Discounts

### Educational Institutions
- **50% discount** on all tiers for accredited schools/universities
- **Free classroom licenses** for educational curriculum use
- Must display "Educational Use" notice in UI

### Non-Profit Organizations
- **30% discount** for registered 501(c)(3) or equivalent
- Must provide proof of non-profit status
- Emergency management agencies qualify

### Amateur Radio Clubs
- **25% discount** for registered amateur radio clubs
- Must be affiliated with ARRL or equivalent national organization
- Cannot be used for commercial broadcast

---

## Early Adopter Program (Limited Time)

### Alpha Early Adopter Pricing (Until FCC Certification)
**Lock in lifetime discounts:**
- **Single Station:** 40% off perpetual licenses
- **Multi-Station:** 40% off perpetual licenses
- **Enterprise:** 40% off perpetual licenses

**Benefits:**
- Locked-in pricing for life (perpetual licenses)
- Free upgrade to certified version when available
- Listed as founding customer (with permission)
- Input on feature roadmap
- Beta access to new features

**Investment:**
- Single Station Perpetual: **$1,797** (reg. $2,995) - **Cheaper than a DASDEC!**
- Multi-Station Perpetual: **$7,797** (reg. $12,995) - **Under $800/installation**
- Enterprise Perpetual: **$29,997** (reg. $49,995) - **Unlimited installations**

---

## Market Context & Value Proposition

### Competitive Analysis

**Traditional EAS Encoder/Decoder Hardware:**
- **DASDEC (Digital Alert Systems):** $3,000 - $7,000
- Sage Digital ENDEC: $7,000 - $12,000
- Gorman-Redlich CAP-DEC: $5,000 - $8,500
- Monroe Electronics R189 OneBox: $6,500 - $9,000
- **Annual licensing/support:** $500 - $2,000

**Cloud-based Alert Services:**
- AlertMedia: $49/user/month = $6,000+/year for 10 users
- Everbridge: Custom pricing, typically $10,000+/year
- OnSolve: $5,000 - $50,000/year depending on volume

### EAS Station Value
- **Price competitive with DASDEC** - Similar upfront cost
- **Open architecture:** No vendor lock-in
- **Self-hosted:** No recurring cloud fees (perpetual license)
- **Customizable:** Full control over features and integration
- **Modern tech stack:** Docker, PostgreSQL, Python, REST APIs
- **Hardware freedom:** Runs on Raspberry Pi ($100) or commodity servers

**Total Cost of Ownership (3 years):**
- **DASDEC:** $5,000 (hardware) + $3,000 (support) = **$8,000**
- **EAS Station Annual:** $995 × 3 = **$2,985** (63% savings)
- **EAS Station Perpetual:** $2,995 + $398 (2 years optional support) = **$3,393** (58% savings)
- **EAS Station Early Adopter:** $1,797 (perpetual) + $398 (2 years support) = **$2,195** (73% savings)

**PLUS: Raspberry Pi hardware (~$250) vs. $5,000 commercial EAS hardware = Additional $4,750 savings**

---

## Licensing Comparison

| Feature | AGPL v3 (Open Source) | Commercial License |
|---------|----------------------|-------------------|
| **Cost** | Free | $995 - $19,995/year |
| **Use in closed-source products** | ❌ No | ✅ Yes |
| **Must share modifications** | ✅ Yes (if deployed as service) | ❌ No |
| **Technical support** | Community only | ✅ Priority support |
| **Warranty** | None | Limited warranty |
| **Custom development** | No | ✅ Available |
| **Remove "Powered by EAS Station"** | ❌ No | ✅ Yes (with attribution in credits) |
| **Embed in proprietary systems** | ❌ No | ✅ Yes |
| **Redistribute to customers** | Only with source code | ✅ OEM license available |

---

## Purchase Process

### How to Buy

1. **Contact:** Email sales@easstation.com with:
   - Your organization name and use case
   - Desired license tier
   - Number of installations needed
   - Any custom requirements

2. **Quote:** Receive formal quote within 1-2 business days

3. **Agreement:** Review and sign commercial license agreement

4. **Payment:** Submit payment via agreed method

5. **Activation:** Receive license key and access within 24 hours

### Trial/Evaluation
- **30-day evaluation license** available (no credit card required)
- Full-featured commercial version
- No obligation to purchase
- Request via sales@easstation.com

---

## FAQ

**Q: What happens if I stop paying the annual subscription?**
A: You can continue using the last version you received, but won't receive updates, bug fixes, or support. Your data remains yours.

**Q: Can I switch from annual to perpetual license?**
A: Yes! Annual payments can be credited toward perpetual license purchase (up to 50% of perpetual price).

**Q: Do I need a license for development/testing?**
A: No, AGPL v3 license is free for internal testing. Commercial license only needed for production deployment or closed-source integration.

**Q: What if I outgrow my tier?**
A: Easy upgrades available anytime. Pay the prorated difference for annual subscriptions.

**Q: Is source code included?**
A: Yes, all licenses include full source code. Enterprise licenses grant modification rights without AGPL obligations.

**Q: When will FCC certification be complete?**
A: Target: Q2-Q3 2026. Early adopter licenses will be upgraded automatically at no additional cost.

**Q: Can I get a refund?**
A: Annual subscriptions: 30-day money-back guarantee. Perpetual licenses: 90-day money-back guarantee if software doesn't meet documented specifications.

---

## Contact Information

**Commercial Licensing Inquiries:**
Timothy Kramer (KR8MER)
Email: sales@easstation.com
Phone: Available upon request for qualified prospects
Website: https://github.com/KR8MER/eas-station

**Sales Hours:**
Monday-Friday, 9 AM - 5 PM Eastern Time
Emergency/urgent inquiries: 24-hour email monitoring

---

## Legal Notes

- All prices in USD
- Prices subject to change with 90-day notice to existing customers
- Volume discounts available for 25+ licenses (contact for quote)
- Government agency pricing available (contact for quote)
- All licenses governed by laws of Ohio, United States
- Export restrictions may apply for international sales

**Tax:** Prices do not include applicable sales tax, VAT, or GST. Tax added at checkout based on location.

---

*Last Updated: November 21, 2025*
*Document Version: 1.0-ALPHA*
