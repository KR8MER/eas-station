...
def _install_certificate_internal(domain: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # chain.pem (issuer chain, no leaf cert) is what OCSP stapling needs
        # to verify the CA's signed "still valid" response -- see the
        # ssl_stapling directives in config/nginx-eas-station.conf. Optional:
        # certbot always writes it alongside fullchain/privkey, but an
        # imported or unusually-obtained cert might lack it, and losing
        # stapling isn't worth failing an otherwise-good cert install over.
        ssl_trusted_certificate_line = (
            f"ssl_trusted_certificate {chain_path};\n" if chain_path.exists() else ""
        )
        if not chain_path.exists():
            logger.info(f"No chain.pem in {cert_dir} -- OCSP stapling will stay inactive for this cert")
